...
export default function Dashboard() {
  ...
  const filtered = useMemo(() => {
    return sessions.filter((s) => {
      ...
      if (filters.year && d.getFullYear() !== Number(filters.year))
        return false;
      if (filters.month && d.getMonth() + 1 !== Number(filters.month))
        return false;
      if (filters.locationId && s.location?.id !== filters.locationId)
        return false;
  ...
  const yearFiltered = useMemo(() => {
    ...
    return sessions.filter(
      (s) => parseLocalDate(s.date).getFullYear() === Number(filters.year),
    );
  ...
  const monthlyByYear = useMemo(() => {
    ...
    if (!filters.year) {
      return computeMonthlyByYear(sessions, {
  ...
  const tempExtremes = useMemo(() => {
    ...
    return {
      max: {
        temp: toF(max.water_temp_c),
        date: max.date,
        location: max.location?.name,
      },
      min: {
        temp: toF(min.water_temp_c),
        date: min.date,
        location: min.location?.name,
      },
    };
  }, [filtered]);

  const waterTempByMonth = useMemo(
    () => computeWaterTempByMonth(filtered),
    [filtered],
  );
      ...
      <div className="p-4 flex flex-col gap-6">
        ...
        {monthlyByYear ? (
          <SurfChart
            ...
            data={monthlyByYear.data}
            bars={monthlyByYear.bars}
          />
        ) : (
          <SurfChart
            title="Monthly Breakdown"
        ...
        {stats.byBoard.length > 0 && (
          <SurfChart
            ...
            multiColor
            logScale
            onBarClick={handleBoardClick}
          />
        )}
        {stats.byFinType.length > 0 && (
          <SurfChart
            title="Fins"
            data={stats.byFinType}
            color="#FF2D78"
            multiColor
            logScale
            onBarClick={handleFinClick}
          />
        )}
        ...
        {stats.byWaveSize.length > 0 && (
          <SurfChart
            title="Wave Height (Observed)"
            data={stats.byWaveSize}
            color="#00CFFF"
          />
        )}
        {stats.bySwellSize.length > 0 && (
          <SurfChart
            title="Wave Height (API)"
            data={stats.bySwellSize}
            color="#BF00FF"
          />
        )}
